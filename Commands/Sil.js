...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  ...
  if(!args[0] || (args[0] && isNaN(args[0])) || Number(args[0]) < 1 || Number(args[0]) > 1000) return message.channel.send(embed.setDescription("1-1000 arasında silinecek mesaj miktarı belirtmelisin!")).then(x => x.delete({timeout: 5000}));
